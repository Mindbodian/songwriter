    ...
    <div className="grid grid-cols-4 grid-rows-2 gap-0.5">
      {beatLabels.map((label, index) => (
        <button
          ...
          className={`w-7 h-7 rounded-sm flex items-center justify-center text-[6px] font-bold transition-all duration-100 ${
            activeLoop === index 
              ? 'bg-amber-600 border border-b-2 border-amber-800 border-t-amber-400 border-l-amber-400 border-r-amber-800 shadow-inner text-white active:border-b active:translate-y-0.5' 
              : 'bg-amber-500 border border-b-2 border-amber-700 border-t-amber-300 border-l-amber-300 border-r-amber-700 text-black hover:bg-amber-400 active:border-b active:translate-y-0.5'
          }`}
        >
